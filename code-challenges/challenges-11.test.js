...
Write a function named validateWord that uses a regular expression pattern
 validate that a word is between 5 and 10 characters long.
...
Write a function named hasNumber that uses a regular expression pattern to 
determine if a string has one or more letter followed by one or more digit.
If it does, return true. If not, return false.
------------------------------------------------------------------------------------------------ */

const hasNumber = (string) => {
  // Solution code here...
  const hasNum = /[a-zA-Z]+[0-9]+/;

  return hasNum.test(string);
};
...
const validateEmail = (email) => {
  // Solution code here...
  const mail = /^[A-Za-z0-9]+.?[A-Za-z0-9]+@[\w]+.(com|net|org)$/
  // let regx = /^\w+([.]?\w+)@.+[.]\w{3}$/g;
