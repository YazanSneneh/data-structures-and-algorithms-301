...
function transformToLis(obj) {
  ...
  let objects = []
  objects.push(obj);
  let returnList = objects.reduce((list, obj) => {
    for (const prop in obj) {
      list.push(`<li>${prop}: ${obj[prop]}</li>`)
    }
    return list;
  }, [])
  return returnList;
...
const addPurchases = (arr) => {
  // Solution code here...
  let totalPrice = arr.reduce((total, val) => {
    total = total + val.purchasePrice;
    return total;
  }, 0)
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let arrayItems = arr.reduce((sum, item) => {
    sum = sum + 1;
    return sum
  }, 0)
  return arrayItems
...
const returnNames = (arr) => {
  // Solution code here...
  let names = arr.reduce((accom, obj) => {
    accom.push(obj.name);
    return accom;
  }, [])
  return names;
...
------------------------------------------------------------------------------------------------ */
/*
 get string and loop over it.
 get length
*/
const reversedString = (str) => {
  // Solution code here...
  let revesed = str.split('').reduce((string, char, idx) => {
    return char + string;
  }, '')
  return revesed
};
